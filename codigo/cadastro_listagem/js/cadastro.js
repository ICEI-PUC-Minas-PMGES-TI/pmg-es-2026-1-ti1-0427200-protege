...
document.addEventListener('DOMContentLoaded', () => {
  ...
  botoesNivel.forEach(botao => {
    botao.addEventListener('click', () => {
      ...
      botoesNivel.forEach(b => b.classList.remove('ativo-familiar', 'ativo-amigos', 'ativo-conhecidos'));
      ...
      // Esconde o erro de nível, se existia
  ...
  formulario.addEventListener('submit', evento => {
    ...
    formularioValido = validarCampo(campoNome,     'erroNome',     valor => valor.trim().length >= 3)                                   && formularioValido;
    formularioValido = validarCampo(campoEmail,    'erroEmail',    valor => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(valor.trim()))            && formularioValido;
    formularioValido = validarCampo(campoTelefone, 'erroTelefone', valor => valor.replace(/\D/g, '').length >= 10)                      && formularioValido;

    // Valida o nível de parentesco
    if (!nivelSelecionado) {
  ...
  document.getElementById('botaoCancelar').addEventListener('click', () => {
    ...
    botoesNivel.forEach(b => b.classList.remove('ativo-familiar', 'ativo-amigos', 'ativo-conhecidos'));
    nivelSelecionado = null;
    document.querySelectorAll('.grupo-campo').forEach(grupo => grupo.classList.remove('campo-invalido'));
  });
